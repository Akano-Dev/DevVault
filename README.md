...
**Pixel notifications** — QuestPanel draws its own toasts rather than handing
them to Windows, so a notification looks like the rest of the app instead of a
rounded system card branded with the host process's name
- Slides into the **top-right corner**, stacking downward, up to three at a time
- Beveled slot, pixel font, item glyph, and a durability bar that drains as it times out
- Its own chiptune (`assets/audio/notify.wav`, generated by `tools/make_sfx.py`)
- Fires when a task timer reaches its target and when an objective is fully checked off
- Click one to bring the panel forward, or the cross to dismiss; hovering holds it open
- Never takes focus, so one arriving mid-sentence cannot eat a keystroke
- One setting switches the whole channel off, and **Settings → Notifications** has a
  test button
